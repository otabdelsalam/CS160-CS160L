...
public class Race extends JFrame {
	...
	private void prepareToRace() {
		...
		while (input != 's' && input != 'S') {
			...
			{
				case 't', 'T':
					Tortoise newT = new Tortoise("Tortoise", START_LINE, yPos);
					racerList.add(newT);
					yPos += RACER_SPACE;
					break;
				case 'h', 'H':
					Hare newH = new Hare("Hare", START_LINE, yPos);
					racerList.add(newH);
					yPos += RACER_SPACE;
					break;
				case 'k', 'K':
					Snake newSnake = new Snake("Snake", START_LINE, yPos);
					racerList.add(newSnake);
					yPos += RACER_SPACE;
					break;
				case 'd', 'D':
					Dog newDog = new Dog("Dog", START_LINE, yPos);
					racerList.add(newDog);
					yPos += RACER_SPACE;
					break;
				default:
	...
	private char getRacer() {
		String input = JOptionPane.showInputDialog(this, "Enter a racer:"
				+ "\nt for Tortoise, h for hare, k for snake, d for dog," + "\nor s to start the race");
		if (input == null) {
